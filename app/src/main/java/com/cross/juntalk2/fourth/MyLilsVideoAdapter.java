...
import com.cross.juntalk2.databinding.AdapterMylilsVideoItemBinding;
...
public class MyLilsVideoAdapter extends ListAdapter<VideoListModel, MyLilsVideoAdapter.MyLilsVideoListAdapterViewHolder> {
    ...
    private MyLilsVideoActivity.RefreshInterface refreshInterface;
    ...
    public void setRefreshInterface(MyLilsVideoActivity.RefreshInterface refreshInterface) {
    ...
    protected MyLilsVideoAdapter(@NonNull @NotNull DiffUtil.ItemCallback<VideoListModel> diffCallback, Context context, Activity activity) {
        super(diffCallback);
    ...
    public void setItems(SimpleExoPlayer player) {
        this.player = player;
    ...
    @NotNull
    @Override
    public MyLilsVideoListAdapterViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        AdapterMylilsVideoItemBinding binding = AdapterMylilsVideoItemBinding.inflate(inflater, parent, false);
        binding.setMyLilsVideoListAdapter(this);
        return new MyLilsVideoListAdapterViewHolder(binding);
    }
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_SETTLING) {
                    ...
                    if (childView != null) {
                        MyLilsVideoListAdapterViewHolder viewHolder = (MyLilsVideoListAdapterViewHolder) childView.getTag();
                        if (viewHolder != null) {
    ...
    @Override
    public void onViewAttachedToWindow(@NonNull MyLilsVideoListAdapterViewHolder holder) {
        ...
        if (holder.getAbsoluteAdapterPosition() == 0) {
            ...
            player.play();
            holder.binding.styledPlayerView.getVideoSurfaceView().setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    toggleVolume(holder.binding.volumeImageView);
                }
            });
    ...
    @Override
    public void onViewDetachedFromWindow(@NonNull MyLilsVideoListAdapterViewHolder holder) {
    ...
    private void toggleVolume(AppCompatImageView volumeImageView) {
        if (volumeState == VolumeState.OFF) {
            setVolumeControl(VolumeState.ON, volumeImageView);
        } else if (volumeState == VolumeState.ON) {
            setVolumeControl(VolumeState.OFF, volumeImageView);
        }
    }

    private void setVolumeControl(VolumeState state, AppCompatImageView volumeImageView) {
        volumeState = state;
    ...
    public void clickLike(View view, VideoListModel videoListModel, int position) {
        ...
        Log.e("abc", "videoList : " + videoListModel.user_Index);
                ...
                if (response.isSuccessful()) {
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            dialogLilsReplyList.setMy_replyClickInterface(replyClickInterface);
    ...
    public void blockLilsVideo(View view, VideoListModel videoListModel, int position) {
        ...
        if (userId.equals(videoListModel.userId)) {
            subTextView.setText("정말로 삭제하시겠습니까?");
            okButton.setText("삭제하기");
            cancelButton.setText("취소하기");
            reportButton.setVisibility(View.GONE);

        } else {
            subTextView.setText("차단하시겠습니까?");
    ...
    @Override
    public void onBindViewHolder(@NonNull @NotNull MyLilsVideoListAdapterViewHolder holder, int position) {
        holder.onBind(getItem(position), holder.getAbsoluteAdapterPosition());
    }

    @Override
    public int getItemViewType(int position) {
        return R.layout.adapter_mylils_video_item;
    }
    ...
    public class MyLilsVideoListAdapterViewHolder extends RecyclerView.ViewHolder {
        public AdapterMylilsVideoItemBinding binding;
        public View parent;
        private String videoUrl;

        public MyLilsVideoListAdapterViewHolder(@NonNull @NotNull AdapterMylilsVideoItemBinding binding) {
            super(binding.getRoot());
            this.binding = binding;
            parent = binding.getRoot();
            this.binding.setMyLilsVideoListAdapterViewHolder(this);

        }

        public void onBind(VideoListModel videoListModel, int absolutePosition) {

            if (userId.equals(videoListModel.userId)) {
                binding.blockImageButton.setImageResource(R.drawable.ic__351953_bin_cancel_close_delete_garbage_icon);
                binding.blockImageButton.setImageTintList(ColorStateList.valueOf(Color.parseColor("#4FC3F7")));
                binding.blockImageButton.getLayoutParams().width = getDp(50);
                binding.blockImageButton.getLayoutParams().height = getDp(50);
                binding.makeVideoListImageButton.setVisibility(View.VISIBLE);

            } else {

                binding.blockImageButton.setImageResource(R.drawable.ic_three_dot);
                binding.blockImageButton.setImageTintList(ColorStateList.valueOf(Color.parseColor("#4FC3F7")));
                binding.blockImageButton.getLayoutParams().width = getDp(50);
                binding.blockImageButton.getLayoutParams().height = getDp(50);
